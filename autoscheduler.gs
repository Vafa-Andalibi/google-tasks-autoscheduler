...
var days_to_plan_ahead = 10;
var delay_between_tasks = (15 * 60 * 1000);
...
function event_creator()
{
  ...
  Logger.log("deleting all current events...")
  ...
  for(var i=0; i<events.length;i++){
    ...
    if (current_task_end_time < today_now)// && current_event.getDescription() == 'one-time')
    {
      ...
      else if (current_event.getDescription() != 'recurrent')
    ...
    else
    {
      ...
      catch (e) {PropertiesService.getScriptProperties().setProperty("run", "STOP"); }
    }
  }
  Logger.log("Old events deleted.")
  ...
  Logger.log("Going through the days ...")
  for (var j=0; j < days_array.length ; j++)
  {
    ...
    while (all_tasks_reviewed == 0)
    {
      ...
      var tmp_task_to_create_event_for = []
      if (one_time_tasks_sorted_indices.length > 0 && days_array[j]['sorted_order'].length > 0)
      {
        ...
        {
          tmp_task_to_create_event_for = all_tasks[one_time_tasks_sorted_indices[0]];
        ...
        {
           tmp_task_to_create_event_for = recurrent_tasks_for_the_day[days_array[j]['sorted_order'][0]];
      ...
      {
        //tmp_task_to_create_event_for = all_tasks[sorted_indices[0]];
        tmp_task_to_create_event_for = all_tasks[one_time_tasks_sorted_indices[0]];
        last_picked_index = one_time_tasks_sorted_indices[0]
        one_time_tasks_sorted_indices.splice(0,1);
        one_time_task_set_flag = 1
            ...
            if (previously_done_events_today.indexOf(tmp_task_to_create_event_for['task_name']) == -1)
            {
              if (previously_done_events.indexOf(tmp_task_to_create_event_for['task_name']) == -1)
              {
                if (tmp_task_to_create_event_for['recurrent_on'] != "")
                  var desc = 'recurrent'
                else if (tmp_task_to_create_event_for['extended_every'] != "")
                  var desc = 'extendable'
                else
                  var desc = 'one-time'
  ...
  for (var i = 0; i < taskLists.items.length; i++){
    ...
    if (tasks.items != undefined)
    {
      for (var j=0; j< tasks.items.length ; j++){
        if (tasks.items[j].status != 'completed' && tasks.items[j].notes!= undefined && (tasks.items[j].notes.toLowerCase().indexOf("pr") > -1) &&
          (tasks.items[j].notes.toLowerCase().indexOf("est") > -1) && tasks.items[j].title.indexOf('#') != 0 ) // # is for commented tasks
        {
          ...
          {
            if (tmp_dict['extended_every'].toLowerCase().indexOf('d') > -1 )
            {
              var days_to_extend = parseInt(tmp_dict['extended_every'].substr(0,recurrency.toLowerCase().indexOf('d')))
              }
            else if (tmp_dict['extended_every'].toLowerCase().indexOf('w') > -1 )
            {
              var days_to_extend = parseInt(tmp_dict['extended_every'].substr(0,recurrency.toLowerCase().indexOf('w')))*7
            }
          ...
          if (json_data.rec != undefined)
          {
            ...
            if (tmp_dict['recurrent_every'].toLowerCase().indexOf('start') > -1) {
              tmp_dict['start_date'] = new Date(recurrency.substr(recurrency.toLowerCase().indexOf('start')+5))
            }
          }
          ...
          if (tmp_dict['divide_into'] != undefined && tmp_dict['divide_into'] != 1)
          {
            ...
            var chunk_time = parseInt(tmp_dict['estimated_time']/ tmp_dict['divide_into'],10)
...
function recurrent_event_finder(all_tasks){
  ...
  {
    if (all_tasks[i]['recurrent_every'].toLowerCase().indexOf('d') > -1 )
    {
      ...
      all_tasks[i]['task_days'] = recurrence_day_seq_finder(all_tasks[i]['start_date'],tmp_rec_on,tmp_rec_every.substr(0,tmp_rec_every.toLowerCase().indexOf('d')),last_day_of_plan,start_day_to_plan)

    }
    else if (all_tasks[i]['recurrent_every'].toLowerCase().indexOf('w') > -1 )
    {
      var tmp_rec_on = all_tasks[i]['recurrent_on'].split('&').map(day_to_number).sort()
      var tmp_rec_every = parseInt(all_tasks[i]['recurrent_every'].substr(0,all_tasks[i]['recurrent_every'].toLowerCase().indexOf('w')))
...
function estimate_to_ms(inp_estimate) {
  if (inp_estimate.toLowerCase().indexOf("h")> -1){
    inp_estimate = parseInt(inp_estimate.replace("h",""));
    return inp_estimate * 60 * 60 * 1000 ;
  }
  else if (inp_estimate.toLowerCase().indexOf("m")> -1){
